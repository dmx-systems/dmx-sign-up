...
public class SignupPlugin extends PluginActivator implements SignupService, PostUpdateTopic, PostLoginUser {
    ...
    private OptionalService<LDAPService> ldapService;
    ...
    private void initOptionalServices() {
        ldapService = new OptionalService<>(getBundleContext(), () -> LDAPService.class);
    }
    ...
    public PasswordChangeRequestResult requestPasswordChange(@PathParam("token") String token,
        ...
        if (tokenData != null) {
            ...
            if (!isLdapAccountCreationEnabled()) {
                ...
                if (ldapService.get().changePassword(newCreds) != null) {
    ...
    private Topic createUsername(Credentials credentials) throws Exception {
        if (isLdapAccountCreationEnabled()) {
            return ldapService.get().createUser(credentials);
        } else {
